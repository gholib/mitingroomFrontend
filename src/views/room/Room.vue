<template>
  <div v-if="$isAdmin()" id="data-list-list-view" class="data-list-container">

    <data-view-sidebar :isSidebarActive="addNewDataSidebar" @closeSidebar="toggleDataSidebar" :data="sidebarData" />

    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="rooms">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
          ...
          <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary" @click="addNewData">
              ...
              <span class="ml-2 text-base text-primary">Добавить комнату</span>
          </div>
        </div>
      ...
      <template slot="thead">
        ...
        <vs-th>Обложка</vs-th>
        <vs-th sort-key="ROOM">Название</vs-th>
...
<script>
import DataViewSidebar from '../../components/room/RoomSidebar'
  ...
  methods: {
    openConfirm(id) {
      ...
      this.$vs.dialog({
        ...
        text: 'Безвозвратное удаление категории.',
    ...
    deleteData () {
      this.$store.dispatch('room/DELETE_ROOM', this).catch(err => { console.error(err) })
    },
    editData (data) {
    ...
    deleteSelected() {
      ...
      if(this.selectedId.length > 0){
        this.$store.dispatch('room/DELETE_SELECTED_ROOMS', this).catch(err => { console.error(err) })
      }
    }
  },
