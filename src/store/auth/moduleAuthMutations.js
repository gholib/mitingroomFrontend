export default {
    SET_TOKEN(state, response) {
        localStorage.setItem('mitingroom-user-token', response[0]);
        localStorage.setItem('mitingroom-user-profile', JSON.stringify(response[1]));
    ...
    UNSET_TOKEN(state) {
        ...
        state.token = null;
        state.user = null;
        localStorage.removeItem('mitingroom-user-token');
        localStorage.removeItem('mitingroom-user-profile');
